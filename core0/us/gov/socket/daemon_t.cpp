...
void c::detach(client& peer) {
    ...
    peer.set_finished_and_shutdown_recv(); //follows client::on_destroy_ called from server thread - update clients - before going to gc
    handler::task_wakeup();
}

void c::attach(client* peer) {
    log("attach. fd", peer->get_sock(), peer->endpoint());
    ...
    */
    ...
    log("setting not_blocking_mode", peer->get_sock());
    auto r = peer->set_not_blocking_mode();
    assert(r == 0);
    peers.add(peer);
    handler::task_wakeup();
}
