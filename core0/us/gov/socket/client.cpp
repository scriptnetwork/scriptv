...
const char* c::KO_10100 = "KO 10100 Unreachable host/Connection refused.";
...
void c::set_finished_and_reset_sock() {
    log("set_finished2");
    set_finished();
    sock = -1;
}

void c::set_finished_and_shutdown_recv() {
    log("set_finished_and_reset_sock");
    finished.store(true);
    if (sock == -1) return;
...
bool c::is_wan_ip(host_t x) {
    if (x == 0xFFFFFFFF) [[unlikely]] return false; //255.255.255.255/32
    x &= 0x00FFFFFF;
    if (x == 0x000000C0) [[unlikely]] return false; //192.0.0.0/24
    if (x == 0x000200C0) [[unlikely]] return false; //192.0.2.0/24
    if (x == 0x006358C0) [[unlikely]] return false; //192.88.99.0/24
    if (x == 0x006433C6) [[unlikely]] return false; //198.51.100.0/24
    if (x == 0x007100CB) [[unlikely]] return false; //203.0.113.0/24
    x &= 0x0000FFFF;
    if (x == 0x0000FEA9) [[unlikely]] return false; //169.254.0.0/16
    if (x == 0x0000A8C0) [[likely]] return false; //192.168.0.0/16
    if ((x & 0x0000FEFF) == 0x000012C6) [[unlikely]] return false; //198.18.0.0/15
    if ((x & 0x0000F0FF) == 0x000010AC) [[unlikely]] return false; //172.16.0.0/12
    if ((x & 0x0000C0FF) == 0x00004064) [[unlikely]] return false; //100.64.0.0/10
    x &= 0x000000FF;
    if (x == 0x00000000) [[unlikely]] return false; //0.0.0.0/8
    if (x == 0x0000000A) [[likely]] return false; //10.0.0.0/8
    if (x == 0x0000007F) [[likely]] return false; //127.*
    x &= 0x000000F0;
    if (x == 0x000000E0) [[unlikely]] return false; //224.0.0.0/4 //Multicast
    if (x == 0x000000F0) [[unlikely]] return false; //240.0.0.0/4 //reserved
    return true;
}
...
hostport_t c::raddress() const {
    ...
    if (a != 0) [[unlikely]] return make_pair(0, 0);
    if (addr.ss_family == AF_INET) [[likely]] {
...
pair<ko, datagram*> c::recv6(busyled_t& busyled) {
    ...
    while(true) {
        ...
        if (r.second->completed()) {
            ...
            if (peer_api_v != daemon.api_v) [[unlikely]] { 
