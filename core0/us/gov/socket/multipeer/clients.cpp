...
ko c::connect0(const hostport_t& hostport) {
    if (hostport.first == 0) [[unlikely]] {
        log(client::KO_60541);
        return client::KO_60541;
    }
    if (hostport.second == 0) [[unlikely]] {
        log(client::KO_58961);
...
void c::add(client* peer) {
    log("add fd", peer->get_sock(), "sz", size());
    lock_guard<mutex> lock(mx);
    assert(peer);
    assert(peer->get_sock() > 0);
    auto i = find(peer->get_sock());
    if (i == end()) [[likely]] {
        emplace(peer->get_sock(), peer);
        log("added fd", peer->get_sock(), "clients_sz", size());
        return;
    }
    log("fd already inn container", peer->get_sock(), "sz", size());
    i->second->set_finished_and_reset_sock(); //forget same fd from a prev connection
    ...
    vector<client*> disconnect;
    {
        ...
        for (auto i = begin(); i != end(); ) {
            ...
            if (c.is_finished()) [[unlikely]] {
                ...
                if (sl.first == client::security_level_ok) [[likely]] {
                    if (c.busyrecv.load() == 0) {
                        log("ready to receive. fd", c.get_sock(), c.endpoint());
                        honest.emplace_back(c.get_sock());
                    }
                    else { //--strip
                        log("not ready to receive. fd", c.get_sock(), c.endpoint()); //--strip
                    } //--strip
                }
