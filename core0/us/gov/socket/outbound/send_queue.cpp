...
ko c::send(datagram* d, client* peer, uint16_t pri) {
    log("admission pri", pri, "sz", d->size(), "seq", d->decode_sequence(), "recipient", peer->endpoint(), "fd", peer->get_sock(), "dgram svc field (to be overwritten)", d->service, "svc", d->decode_service());
    assert(d->completed());
    log("overwriting service field with pri", pri);
    d->service = pri;
    unique_lock<mutex> lock(mx);
    if (size() >= wmh) [[unlikely]] {
        delete d;
...
void c::run_send() {
    ...
    while(isup()) {
        ...
        if (d->second->sendref.load() < 0) [[unlikely]] { //was: !d->second->sendref.load()<0   ???
            log("client dying", d->second, "dgram progress was", d->first->dend, "/", d->first->size());
            d->first->dend = d->first->size();
            continue;
        }
        if (d->second->get_sock() < 0) [[unlikely]] {
            log("sock? fd=", d->second->get_sock(),". dgram progress was", d->first->dend, "/", d->first->size());
            d->first->dend = d->first->size();
            continue;
        }
        busyled.set();
        int n = ::send(d->second->get_sock(), d->first->data() + d->first->dend, sz, MSG_NOSIGNAL);
        busyled.reset();
        if (n >= 0) [[likely]] {
            ...
            d->errcount = 0;
            log("::send fd", d->second->get_sock(), "sz", sz, "returned", n);
            d->first->dend += n;
            ...
            log("::send fd", d->second->get_sock(), "sz", sz, "returned", n, errno, strerror(errno));
        }
        #if CFG_COUNTERS == 1
            ++dropped; //issues
        #endif
        ...
        switch (errno) {
            case EINPROGRESS:
            case EAGAIN:
                log("KO 93216 temp error EAGAIN");
                hold(d->second);
                if (++d->errcount >= 255) {
                    log("KO 93217 disconnect after errcount maxed.");
                    d->second->disconnect(0, "");
                }
                continue;
            case EBADF: //9 Bad file descriptor
            case EPIPE: //32 Broken pipe
                log("KO 93218 EBADF(9) EPIPE(32) handler. errno", errno, strerror(errno));
                d->second->disconnect(0, "");
            default:
