...
bool c::process(const evidence& e) {
    log("process evidence. eid", +e.eid);
...
void c::process(const wallet_address& t) {
    ...
    if (t.pkh.is_zero()) [[unlikely]] {
        auto r = "KO 34029 invalid wallet address.";
        log(r);
        return;
    }
    if (!us::gov::socket::client::is_valid_ip(t.net_addr, demon.peerd.channel)) [[unlikely]] {
        auto r = "KO 34030 invalid address.";
        log(r, t.net_addr, demon.peerd.channel);
        return;
    }
    if (t.net_addr == 0) [[unlikely]] {
        auto r = "KO 34031 invalid null address.";
        log(r);
        return;
    }
    if (t.pport == 0) [[unlikely]] {
...
void c::import(const wallets_t& online) {
    ...
    for (auto& i: online) {
        if(i.second.net_address == 0) [[unlikely]] {
