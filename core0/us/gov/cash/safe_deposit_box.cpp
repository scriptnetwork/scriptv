...
ko c::merge(const safe_deposit_box& other) {
    value += other.value;
    if (other.m != nullptr) {
        if (m != nullptr) {
            m->merge(*other.m);
...
void c::set_supply(const hash_t& token, const cash_t& supply) {
    ...
    auto r = t->set_supply(token, supply);
    if (is_ko(r)) {
        if (n) {
            delete t;
            t = nullptr;
...
hash_t c::file_hash(const string& component) const {
    if (likely(f == nullptr)) return hash_t::zero_;
...
bool c::burn(const hash_t& addr, const hash_t& token, const cash_t& amount, cash_t& fee) {
        fee += burn_fee;
        if (token.is_zero()) { ///GAS
            auto total_burn = amount + burn_fee;
            if (t != nullptr) {
                if (t->is_sealed()) {
                    if ((value - total_burn) < CFG_LOW_GAS_LEVEL) {
                        log("KO 85009 Address constains a sealed mint. Gas balance would drop below min level.");
                        return false;
                    }
                }
            }
        ...
        return t->burn(addr, token, amount);
}
