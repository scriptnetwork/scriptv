...
namespace us::gov::cash {
    ...
    struct account_t: io::seriable {
        ...
        void on_destroyX(accounts_t& ledger);
        ko merge(const account_t&);
        void dump(const string& prefix, int detail, ostream&) const; //for merged accounts. detail: 0=only gas, 1=1 liner, 2=content
        void dump(const string& prefix, const hash_t& addr, int detail, ostream&) const;  //detail: 0=only gas, 1=1 liner, 2=content
        ...
        cash_t get_value_oil() const;

        cash_t input_eligible(const hash_t& coin) const; //not eligible if account contains data, or not enough gas
        pair<ko, cash_t> input_eligible3(const hash_t& coin) const; //not eligible only if not enough gas
