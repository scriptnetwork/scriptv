...
ko c::authorize(const pub_t& p, pin_t pin, request_data_t& request_data) {
    log("authorize?", endpoint(), p, pin);
    if (engine_daemon().db->auth_app->node_pubkey != p) [[unlikely]] {
        return ok; //Any pubkey is welcome in the public protocol
    }
    log("sysop pubkey->sysop_intent", (is_role_sysop() ? "yes" : "no"));
    if (is_role_sysop()) [[unlikely]] {
...
ko c::verification_completed(pport_t rpport, pin_t pin, request_data_t& request_data) {
    log("verification_completed", endpoint(), "rpport", rpport, "pin", pin);
    {
    ...
    if (is_role_peer() || is_role_sysop()) {
        ...
        if (gov::auth::peer_t::stage != gov::auth::peer_t::authorized) [[unlikely]] { //looks like redundant, handled above
...
//            if (!static_cast<gov::peer::daemon_t&>(daemon).grid.add(*this, true)) {
            if (!static_cast<gov::peer::daemon_t&>(daemon).clique.add(*this, true)) {
                ...
                gov::auth::peer_t::stage = gov::auth::peer_t::denied;
                log(r, "disconnect-grid doesn't accept", endpoint());
                disconnect(0, r);
                return r;
            }
            log("added to grid");
    ...
    if (is_role_device()) {
        ...
        if (!static_cast<gov::peer::daemon_t&>(daemon).grid_dev.add(*this, false)) {
            ...
            log(r, "disconnect-grid_dev doesn't accept", endpoint());
            disconnect(0, r);
            return r;
        }
