...
ko c::handle_fetch_accounts(addresses_t&& addresses, accounts_t& accounts) {
    ...
    if (unlikely(db.accounts->empty())) {
        auto r = "KO 30991 Public ledger is not available. gov daemon may not be in sync. Check 'gov cmd y'.";
        log(r);
        return r;
    }
    for (auto& i: addresses) {
        ...
        if (likely(a != db.accounts->end())) {
            ...
            accounts.emplace(i, us::gov::cash::account_t());
        }
    }
    if (accounts.empty()) {
        auto r = "KO 30992 Address/es not found in public ledger.";
        log(r);
        return r;
    }
    return ok;
