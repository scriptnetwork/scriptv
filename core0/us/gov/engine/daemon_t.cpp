...
bool c::ready_for_consensus() const {
    if (!db->auth_app->is_node()) [[unlikely]] return false;
    if (!syncd.in_sync()) [[unlikely]] return false;
    return true;
}
...
bool c::wait_for_secs_to(const seconds& s, ts_t block_deadline) {
    ...
    if (now >= deadline) [[unlikely]] return true;
    cv_cyc.wait_for(lock, nanoseconds(deadline - now), [&] {
        ts_t now = duration_cast<nanoseconds>(system_clock::now().time_since_epoch()).count();
        if (now >= deadline)  [[likely]] {
            return true;
        }
        if (t::isdown()) [[unlikely]] {
            return true;
        }
        return false;
    });
...
void c::run() {
    ...
    while (t::isup()) {
        ...
        ts_t id{0};
        while (true) {
            if (!ready_for_consensus()) [[unlikely]] {
            ...
            if (_local_deltas == nullptr) [[unlikely]] {
            ...
            if (pools.add(_local_deltas, true)) [[unlikely]] {
                log("");
                set_tx_status(from, id, evt_craftblock);
                write_status("collecting deltas");
                if (!wait_for_secs_to(limit_local_deltas, calendar.block_closure)) [[unlikely]] {
                    log("KO 90594");
                    break;
                }
        ...
        if (!wait_for_secs_to(limit_votes, calendar.block_closure)) [[unlikely]] {
            log("");
            break;
        }
        #if CFG_COUNTERS == 1
        ...
        if (last_consensus.is_zero()) [[unlikely]] {
...
void c::update_peers_state() {
    peerd.visit([&](socket::client& c)->void {
           ...
           if (p.stage == peer_t::sysop) [[unlikely]] return;
           if (p.stage == peer_t::device) [[unlikely]] return;
...
void c::save_evidence(const evidence& ev, ko calendar_result) {
    ...
    if (is_ko(calendar_result)) [[unlikely]] {
...
void c::clear_() {
    ...
    set_last_delta_imported_(hash_t::zero_, 0);
    depth = 0;
}
...
hash_t c::save_db() {
    ...
    ostringstream osf;
    osf << blocksdir << '/' << h;
    if (::rename(file.c_str(), osf.str().c_str()) != 0) [[unlikely]] {
