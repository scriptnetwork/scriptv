...
ko c::push_KO(ko msg, const filter_t& filter) {
    ...
    string m = msg;
    auto blob = io::blob_writer_t::make_blob(m);
    schedule_push(peer_t::push_in_t::get_datagram(daemon.channel, 0, hash_t::zero_, push_ko, blob), filter);
    return msg;
}
...
ko c::push_OK(const string& msg, const filter_t& filter) {
    log("pushing OK:", msg);
    auto blob = io::blob_writer_t::make_blob(msg);
    schedule_push(peer_t::push_in_t::get_datagram(daemon.channel, 0, hash_t::zero_, push_ok, blob), filter);
    return ok;
}
