...
//desktop
//string c::localip("192.168.0.250"); //dont use 127.0.0.1 , unreachable from mobile when using advertised IPs (IP lookup)

//laptop
string c::localip("192.168.0.44"); //dont use 127.0.0.1 , unreachable from mobile when using advertised IPs (IP lookup)
...
//cout << "==============================================================create_node " << r2rhome << endl;
    if (!us::gov::io::cfg0::dir_exists("nodes/" + id)) {
        cerr << "KO 68403 missing conf dir 'nodes/" << id << "' for node " << id << endl;
//cout << "==============================================================DIR DOENT EXIST nodes/" << id << endl;
        assert(false);
    }
...
void c::gov_cli_start() {
    cout << "gov_cli_start local_ip " << localip << endl;
    govx_t::params p;
    p.daemon = false;
    p.port = gov->p.pport;
    p.sysophost = localip;
...
void c::test_balance() {
    {
        ...
        string bal;
        auto t = wallet_cli->rpc_daemon->get_peer().call_balance(2, bal);
        if (is_ko(t)) {
            cout << t << endl;
            assert(false);
        }
    ...
    }
    {
        ...
        account_t account;
        auto r = wallet_cli->rpc_daemon->get_peer().call_merge_accounts(account);
        if (is_ko(r)) {
            cout << wallet_cli->rewrite(r) << endl;
            assert(false);
        }
        cout << "detail 0" << endl;
        account.dump("", 0, cout);
        ...
        cout << "detail 2" << endl;
        account.dump("", 2, cout);
        assert(account.box.value > 0);
    }
//    assert(false);
}

void c::online_node_tests() {
    test_list_protocols();
    test_tx_decode();
    test_balance();
}
...
void c::test_tx_decode(const string txb58) {
    vector<uint8_t> blob_in;
    blob_in = us::gov::crypto::b58::decode(txb58);
    string tx_pretty;
    auto r = wallet_cli->rpc_daemon->get_peer().call_tx_decode(blob_in, tx_pretty);
    if (is_ko(r)) {
        assert(false);
        return;
    }
    assert(!tx_pretty.empty());
    cout << tx_pretty << '\n';
}

void c::test_tx_decode() {
    test_tx_decode(""); //transfer XcbCkbkeVXBJc2jpTJC2Uo4sinL 500 3hy7dKDBGSPSUUSZiUCJpaXcJ2kG
    test_tx_decode("56wu62GdKVt6hp8gt9C1QjE2hqGqaFLnLHZMKM4Lpprd6MrDe6SH74yXqVMBq3X9qq7RXdQ4gENpWMfjRaDLfaP2XZMxMpbfti1MCR8FEvVzjHjEG1tvh6Cp8cZqX4p3y1rVfhzY44Q5Hb46E2EW5JhmatANF53i4YoZJJ4RQvfwYCeD2AzbmjTrYvZkabQgjt8YfcgLCofeCZYu9Kqkcegas"); //set_supply 3hy7dKDBGSPSUUSZiUCJpaXcJ2kG 1000
    test_tx_decode(""); // transfer 3hy7dKDBGSPSUUSZiUCJpaXcJ2kG 1000000 gas
}

hash_t c::create_coin(int64_t supply, bool seal) {
    tee("create_coin", supply, seal, "trace BXmAJ");
    hash_t coin;
    {
    ...
    while (true) {
        blob_t blob_ev;
        auto t = wallet_cli->rpc_daemon->get_peer().call_set_supply(us::wallet::cli::rpc_peer_t::set_supply_in_t(coin, supply, 1), blob_ev); // 1=relay
        if (is_ko(t)) {
            cout << t << endl;
            cout << "lasterror: " << gov_cli->rpc_daemon->get_peer().lasterror << endl;
        ...
        cout << blob_ev.size() << endl;
        assert(!blob_ev.empty());
        {
            string s;
            assert(is_ok(wallet_cli->rpc_daemon->get_peer().call_tx_decode(blob_ev, s)));
            cout << s << endl;
        }
        break;
    }

    if (seal) { // =============================test branch- seal============================
        tee("Sealing mint", coin);
        blob_t blob_ev;
        auto t = wallet_cli->rpc_daemon->get_peer().call_mint_seal(us::wallet::cli::rpc_peer_t::mint_seal_in_t(coin, 1), blob_ev); // 1=relay
        assert(!blob_ev.empty());
        //TODO:test sealing. (supply cannot be reset after sealing)
    }
    {
        ostringstream cmd;
        cmd << "data " << coin;
        while(true) {
            ...
            assert(r.first == ok);
            cout << r.second << endl;
            if (r.second.find(". Mint") != string::npos) {
                tee("coin on ledger!");
                if (seal) {
                    if (r.second.find(us::gov::cash::t_t::msg_sealed) != string::npos) {
                        tee("And immutable!, we're done waiting.");
                        break;
                    }
                    else {
                        cerr << "Mint should be immutable. waiting.\n";
                    }
                }
                else {
                    tee("we're done waiting.");
                    break;
                }
            }
            this_thread::sleep_for(1s);
