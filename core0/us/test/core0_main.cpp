...
//        string datadir = datadir + "/blocks_v8"
    ...
    void serialization() {
        ...
        }
        {
            ...
            if (!b) {
                cout << "#######################################################\n";
                cout << "You've increased the serial_version, " << fn.str() << " directory has been created (later when all tests are ok it must be git controlled).\n";
                {
                ostringstream cmd;
                cmd << "mkdir -p " << fn.str() << "/blocks";
    ...
    bool blob_serialization__new_version{false};
};
...
struct network_c0: network {
    ...
    void stage1_configure() override {
        ...
        {
            auto bank1 = dynamic_cast<node_bank*>(find("bank1")->second);
            auto bank2 = dynamic_cast<node_bank*>(find("bank2")->second);
            hash_t coin;
            {
                coin = bank1->create_coin(1000, true);
                /* console w bank1:
                wallet> balance 3

                24ZUa9QnygEmNjhiNjP2XKiPy5L OIL 2749998992
                3rMRfSKgCnGpWTqyJYxNNhNiwU3s OIL 991; 1 coins
                    Coins:
                        coin 3rMRfSKgCnGpWTqyJYxNNhNiwU3s 1000. Mint. Sealed.
                */
            }
            tee("coin ", coin);

            {
                hash_t rcpt = bank1->gov->daemon->id.pub.hash();
                blob_t blob;
                auto t = bank1->wallet_cli->rpc_daemon->get_peer().call_transfer(us::wallet::cli::rpc_peer_t::transfer_in_t(rcpt, 100, coin, 1), blob);
                if (t != ok) cout << t << endl;
                assert(t == ok);
                ...
                {
                string s;
                assert(is_ok(bank1->wallet_cli->rpc_daemon->get_peer().call_tx_decode(blob, s)));
                cout << s << endl;
                }
            }
...
void test_l2_main(string logdir0) {
    ...
    cin >> inputip;
    if (!inputip.empty()) {
	node::localip = inputip;
    }
    cout << "Using IP address " << node::localip << endl;
...
void help(ostream& os) {
    ...
    os << "--shell                  Bootstrap network and start interactive shell. Android tests.\n";
