...
void c::test_trade_start_dialog_a(node& bid, node& ask, hash_t& trade_id, int n1_num_protocols_seq0, int n1_num_protocols_seq1, int n2_num_protocols_seq0) {
    ...
    bid.wallet_cli_dis->expected_code.check.emplace(us::wallet::trader::trader_t::push_data,
        [&](const hash_t& tid, uint16_t code, const vector<uint8_t>& blob) {
            ...
            assert(tid == trade_id);
            Check_s_contains(payload, "created 17");
            Check_s_contains(payload, "activity 17");
...
void c::test_trade_start_dialog_b_2(node& n1, node& n2, hash_t& trade_id, int num_protocols0, int num_protocols1) {
    log("test_trade_start_dialog_b_2");
    ...
    n1.wallet_cli_dis->expected_code.emplace(us::wallet::trader::trader_t::push_data, 3);
    n1.wallet_cli_dis->expected_code.check.emplace(us::wallet::trader::trader_t::push_data,
        [&](const hash_t& tid, uint16_t code, const vector<uint8_t>& blob) {
            ...
            if (seq == 0) {
                ...
                if (payload.find("state online") != string::npos) { //possible data coming before
                    ++seq;
                    n1.wallet_cli_dis->expected_code.decrement(us::wallet::trader::trader_t::push_data);
                    tee("decrement push_data0");
                }
                else {
                    Check_s_contains(payload, "state offline");
            ...
            if (seq == 1) {
                ...
                Check_s_contains(payload, "peer_protocols 2");
                Check_s_contains(payload, "initiator Y");
                Check_s_contains(payload, "protocol co2vc co");
                Check_s_contains(payload, "logo N");
                Check_s_contains(payload, "chat_script 0");
                Check_s_contains(payload, "homemode me/peer");
                Check_s_contains(payload, "personality_sk ");
...
//                Check_s_contains(payload, "rf N");
                ...
                Check_s_contains(payload, "peer_moniker anonymous");
                Check_s_contains(payload, "pphome");
                Check_s_contains(payload, "/123/wallet/trader/co2vc/co/11111111111111111111/11111111111111111111");
                Check_s_contains(payload, "datadir ");
                Check_s_contains(payload, string("/123/wallet/trader/trade_data/") + to_string(trade_id));
            }
            if (seq == 2) {
    ...
    n2.wallet_cli_dis->expected_code.emplace(us::wallet::trader::trader_t::push_data, 2);
    n2.wallet_cli_dis->expected_code.check.emplace(us::wallet::trader::trader_t::push_data,
        [&](const hash_t& tid, uint16_t code, const vector<uint8_t>& blob) {
            ...
            if (seq == 0) {
                if (payload.find("state online") != string::npos) {
                    ++seq;
                    n2.wallet_cli_dis->expected_code.decrement(us::wallet::trader::trader_t::push_data);
                }
                else {
                    Check_s_contains(payload, "state offline");
                }
            ...
            if (seq == 1) {
                ...
                Check_s_contains(payload, "peer_protocols ");
                Check_s_contains(payload, "initiator N");
                Check_s_contains(payload, "protocol co2vc vc");
                Check_s_contains(payload, "pphome");
                Check_s_contains(payload, "test_l2/home/node_vc/123/wallet/trader/co2vc/vc/11111111111111111111/11111111111111111111");
                Check_s_contains(payload, "datadir ");
                Check_s_contains(payload, string("test_l2/home/node_vc/123/wallet/trader/trade_data/") + to_string(trade_id));
                Check_s_contains(payload, "logo N");
                Check_s_contains(payload, "chat_script 0");
                Check_s_contains(payload, "homemode me/peer");
                Check_s_contains(payload, "personality_sk ");
    ...
    ostringstream cmd;
    assert(!n1.wallet_cli->cur.is_zero());
    cout << "tid: " << n1.wallet_cli->cur << endl;
    cmd << "start co2vc vc";
    cout << cmd.str() << endl;
    exec_cur_trade(n1, cmd.str());

    wait(n1, n2);

    Check_data(n1, "protocol", "co2vc co");
    Check_data(n2, "protocol", "co2vc vc");

    {
        auto s1 = get_data(n1, "local_endpoint");
        auto s2 = get_data(n2, "remote_endpoint");
        cout << "bid local_ep " << s1 << "; ask remote_ep " << s2 << endl;
        assert(s1 == s2);
    }
    {
        auto s1 = get_data(n1, "remote_endpoint");
        auto s2 = get_data(n2, "local_endpoint");
        cout << "bid remote_ep " << s1 << "; ask local_ep " << s2 << endl;
        assert(s1 == s2);
    }
}
