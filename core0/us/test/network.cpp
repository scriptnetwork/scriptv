...
ko c::android_app_test() {
    ...
    cout << "    Name: Connect to wallet\n";
    cout << "    Steps:\n";
    cout << "             0.- Action: Decide subhome (Menu #1) (root wallet=empty; custodial=<string>." << endl;
    cout << "             1.- App. Action: Connect to wallet at : " << node::localip << ":" << n.wallet->p.listening_port << " channel " << 123 << endl;
    cout << "             2.- App. Expect message App: KO10000 Disconnected by peer. Reason given: KO 30291 Device not paired." << endl;
    cout << "             3.- Action: Menu #3" << endl;
    cout << "             4.- App. Action: Reconnect with PIN. (or answer ok to proceed without PIN)" << endl;
    cout << "             5.- App. Expected message: online with " << node::localip << ":" << n.wallet->p.listening_port << " channel " << 123 << endl;
//    cout << "First connect without PIN to receive an error not-authorized." << endl;

    cout << "------------------------------" << endl;
    cout << "Test case #1" << endl;
    cout << "    Name: Check Balance\n";
    cout << "    Steps:\n";
    cout << "             0.- Action: Menu|Balance\n";
    cout << "             1.- App. Expect: OIL 13500000,000" << endl;
    cout << "------------------------------" << endl;

    cout << "------------------------------" << endl;
    cout << "Test case #2" << endl;
    cout << "    Name: My Bookmarks\n";
    cout << "    Steps:\n";
    cout << "             0.- Action: Menu|Bookmarks\n";
    cout << "             1.- App. Expect: Tab My Bookmarks; empty list." << endl;
    cout << "             2.- Action: Tab NC Wallets\n";
    cout << "             3.- App. Expect: Tab NC Wallets; 2 item list." << endl;
    cout << "------------------------------" << endl;
...
void c::consoles_shell() {
    ...
    while(true) {
        ...
        cout << "\ntype exit for repeating the test.\n";
        cout << "select node> "; cout .flush();
        string cmd;
        cin >> cmd;
        cout << "\ncaptured input: '" << cmd << "'\n";
        if (cmd == "exit") {
...
void c::start_govs(bool make_stage) {
    ...
    while (!check()) {
        ...
        ++n;
        cout << "Waiting for all nodes registration in the blockchain. In " << (1 + to - n) << " secs I'd enter an interactive shell.\n";
        if (n > to) {
            n = 0;
            setup_signals(this, false);
            consoles_shell();
...
void c::stage1_create() {
    ...
    while (true) {
        ...
        assert(is_ok(r));
        assert(w.size() > 0);
        cout << "wait reg all wallets. " << w.size() << "/" << size() << '\n';
...
void c::stage1_ff() {
    cout << "Fast forwarding stage1. homedir=" << homedir << '\n';
    {
        ostringstream os;
        os << "mkdir -p " << homedir;
        system(os.str().c_str());
