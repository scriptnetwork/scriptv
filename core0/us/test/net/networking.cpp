...
void c::send_bytes(size_t howmany, uint32_t decl_sz, uint16_t channel, uint16_t svc, bool slowly) {
    ...
    if (slowly) {
        while(tot < howmany) {// 1 sec per chunk
            tee("write fd", p->get_sock(), "tot sofar", tot, "remaning", howmany - tot);
            int b = 1;
            if (tot < datagram::h) {
                if (howmany > datagram::h) {
                    ln = ::write(p->get_sock(), w + tot, datagram::h - tot);
                }
                else {
                    ln = ::write(p->get_sock(), w + tot, 1);
                }
            }
            else {
                ln = ::write(p->get_sock(), w + tot, 1);
        ...
        while (tot < howmany) {
            tee("write fd", p->get_sock(), "tot sofar", tot, "remaning", howmany - tot);
            ln = ::write(p->get_sock(), w + tot, howmany - tot);
            if (ln < 0) {
                if (errno == EAGAIN) {
                    static int times = 0;
                    log("Eagain",times);
                    ...
                    tee("error", errno, strerror(errno)); //server disconnects if channel mismatch, or encoded size <h
                    break;
                }
            }
            tot += ln;
