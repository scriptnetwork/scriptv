...
namespace us::wallet::wallet {
    ...
    struct algorithm: unordered_map<hash_t, keys_t> {
        ...
        ko balance(account_t&) const; //all accounts merged in one
        ko balance(accounts_t&) const;
        void dump_balances(ostream&) const;
        void extended_balance(int detail, ostream&) const;
        ko get_account(const hash_t& address, account_t&) const;
    ...
    public:
        ...
        accounts_t* data{nullptr}; //from ledger
        accounts_t* pdata{nullptr}; //private accounts
