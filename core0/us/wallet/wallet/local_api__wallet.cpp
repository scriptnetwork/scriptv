...
#include <us/wallet/wallet/timeseries_entry_index_t.h>
...
ko c::handle_trade(trade_in_dst_t&& o_in, hash_t& tid) {
    ...
    auto r = businesses.initiate(o_in.parent_trade, o_in.datasubdir, move(qr)); //<orma flow dst="trades_t::initiate"/>
    if (is_ko(r.first)) {
        return r.first;
    }
    tid = r.second;
...
ko c::handle_timeseries_show(timeseries_show_in_dst_t&& o_in, string& ans) {
    ...
    if (o_in.ts == 0) {
        //list files
        ostringstream os;
        os << "cd " << tshome << "; find . -maxdepth 1 -type f";
        return us::gov::io::system_command(os.str(), ans);
    }
    vector<uint8_t> v;
    {
...
ko c::handle_cert_create(string&& msg, hash_t& nft) {
    ...
    cert_t::options o;
    o.xhours = 300000;
    return traders.cert_create(move(msg), move(o), nft);
}

ko c::handle_cert_import(cert_t&& cert, hash_t& nft) {
    log("cert_import");
    return traders.cert_import(move(cert), nft);
}
...
ko c::handle_list_businesses(protocols_t& protocols) {
    log("list_businesses");
    businesses.published_protocols(protocols, false);
    return ok;
}

ko c::handle_accounts(accounts_t& accounts) {
    log("accounts");
    {
        ko r = refresh_data();
        if (unlikely(is_ko(r))) {
    ...
    return w::balance(accounts);
}

ko c::handle_merge_accounts(account_t& merged_account) {
    log("merge_accounts");
    {
        ko r = refresh_data();
        if (unlikely(is_ko(r))) {
    ...
    return w::balance(merged_account);
}

ko c::handle_mint_seal(mint_seal_in_dst_t&& o_in, blob_t& blob_ev) {
    log("mint_seal");
    /// in:
    ///     hash_t addr;
    ///     uint8_t relay;
    ...
    auto tx = algorithm::set_supply(o_in.addr, -1);
    if (is_ko(tx.first)) {
        assert(tx.second == nullptr);
        return tx.first;
    }
...
ko c::handle_udp_send(udp_send_in_dst_t&& o_in) {
    log("udp_send");
    /// in:
    ///     hash_t chan;
    ///     vector<uint8_t> stream_chunk;

    auto r = "KO 10210 Not implemented";
    log(r);
    return r;
}

ko c::handle_data_sources(data_sources_index_t& ans) {
    log("data_sources");
    return daemon.data_sources.get_index(ans);
}

ko c::handle_data_source_connect(data_source_connect_in_dst_t&& o_in, string& result) {
    log("data_source_connect");
    /// in:
    ///     string name;
    ///     hash_t address;
    return daemon.data_sources.connect(o_in.name, o_in.address);
}

ko c::handle_timeseries_list1(timeseries_index_t& ans) {
    log("timeseries_list1");
    ostringstream os;
    os << "find " << home << "/accounts/ -type f -name \"params\" | sed \"s~.*" << home << "/accounts/\\(.*\\)/timeseries/params~\\1~\"";
    string s;
    auto r = us::gov::io::system_command(os.str(), s);
    if (is_ko(r)) {
        return r;
    }
    istringstream is(s);
...
ko c::handle_timeseries_new1(uint8_t&& monotonic, hash_t& addr) {
    log("timeseries_new1");
    addr = algorithm::new_address();
    if (unlikely(addr.is_zero())) {
        auto r = "KO 20919 Invalid new address.";
    ...
    {
        ostringstream os;
        os << home << "/accounts/" << addr << "/timeseries";
        tshome = os.str();
        us::gov::io::cfg0::ensure_dir(tshome);
        params_file = tshome + "/params";
    }
    ...
    {
        ofstream os(params_file, ios::trunc);
        os << (monotonic == 0 ? "free" : "monotonic") << '\n';
    }
    //ostringstream os;
    //os << "New " << (monotonic == 0 ? "free" : "monotonic") << " timeseries created in address " << addr << '\n';
    //ans = os.str();
    return ok;
}

ko c::handle_timeseries_add_local_cert(timeseries_add_local_cert_in_dst_t&& o_in, string& ans) {
    log("timeseries_add_local_cert");
    /// in:
    ///     hash_t addr;
    ///     hash_t nft;
    cert_t cert;
    auto r = traders.ca.cert_get(o_in.nft, cert);
    if (is_ko(r)) {
        return r;
    }
    timeseries_add_in_dst_t dst;
    dst.addr = o_in.addr;
    cert.write(dst.payload);
    return handle_timeseries_add(move(dst), ans);
}

ko c::handle_timeseries__get_cert(timeseries__get_cert_in_dst_t&& o_in, cert_t& ans) {
    log("timeseries__get_cert");
    /// in:
    ///     hash_t addr;
    ///     uint64_t ts_from;
    ///     uint64_t ts_to;
    ...
    timeseries_t timeseries(tshome); {
        auto r = timeseries.load(o_in.ts_from, o_in.ts_to);
        if (is_ko(r)) return r;
    }
    log("timeseries size", timeseries.size());
    hash_t nft;
    cert_t::options o;
    o.xhours = 300000;
    o.kv.set("ẗs_from", o_in.ts_from);
    o.kv.set("ẗs_to", o_in.ts_to);
    auto r = traders.ca.cert_create(timeseries.payload(), move(o), nft);
    if (is_ko(r)) return r;
    return handle_cert_get(move(nft), ans);
}

ko c::handle_trades_index(trades_index_t& index) {
    log("trades_index");
    return traders.trades_index(index);
}

ko c::handle_account(hash_t&& address, account_t& account) {
    log("account");
    {
        ko r = refresh_data();
        if (unlikely(is_ko(r))) {
    ...
    return w::get_account(address, account);
}

ko c::handle_timeseries_entry_index(hash_t&& addr, timeseries_entry_index_t& ans) {
    log("timeseries_entry_index");
    if (addr.is_zero()) {
        return "KO 84120 Invalid address.";
    }
    string tshome;
    ...
    for (auto& p: fs::directory_iterator(tshome)) {
