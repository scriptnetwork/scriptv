...
c::local_api(engine::daemon_t& daemon, const string& home, const string& subhome, const hash_t& subhomeh, trader::endpoint_t&& ep):
        ...
        businesses(*this),
        traders(*this) {
    us::gov::io::cfg0::ensure_dir(home + "/accounts");
    businesses.init(daemon.trades.default_protocols(!subhome.empty())); //<orma flow dst="libs.create_business"/>
...
void c::published_bookmarks(bookmarks_t& b) const {
    ...
    ep.wloc = subhome;
    {
        ostringstream name;
        name << "bm_" << ++n;
        b.add(name.str(), bookmark_t(qr_t(ep), bookmark_info_t("Any role", blob_t())));
        log("added", name.str());
    }
    for (auto& i: v) {
        ostringstream name;
...
string c::timeseries_payload(const hash_t& addr) const {
    timeseries_show_in_dst_t o_in;
    ///     hash_t addr;
    ///     uint64_t ts;
    o_in.addr = addr;
    o_in.ts = 0;
    string ans;
    auto r = const_cast<c&>(*this).handle_timeseries_show(move(o_in), ans);
    if (is_ko(r)) {
        return r;
    }
    return ans;
...
string c::timeseries_payload(const hash_t& addr, ts_t ts_from, ts_t ts_to) const {
    log("timeseries_payload. ts_from", ts_from, "ts_to", ts_to);
    timeseries__get_cert_in_dst_t o_in;
    o_in.addr = addr;
    o_in.ts_from = ts_from;
    o_in.ts_to = ts_to;
    cert_t cert;
    auto r = const_cast<c&>(*this).handle_timeseries__get_cert(move(o_in), cert);
    if (is_ko(r)) {
        return r;
    }
    return cert.as_string();
}
