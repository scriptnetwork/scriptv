...
c::algorithm(const string& keys_filename): keys_filename(keys_filename) {
...
ko c::balance(account_t& account) const {
    lock_guard<mutex> lock(mx);
    if (data == nullptr) {
        return KO_9372;
    }
    return data->merge(account);
}

ko c::balance(accounts_t& accounts) const {
    lock_guard<mutex> lock(mx);
    if (data == nullptr) {
        return KO_9372;
    }
...
ko c::get_account(const hash_t& address, account_t& account) const {
    lock_guard<mutex> lock(mx);
    if (data == nullptr) {
        return KO_9372;
    }
    auto i = data->find(address);
    if (i == data->end()) {
        return "KO 44030 Account not found.";
    }
    account = i->second;
    return ok;
}
...
c::io_accounts_t c::select_destinations_algR0(const cash_t& amount, const hash_t& token) const {
    ...
    for (accounts_t::const_iterator i = data->begin(); i != data->end(); ++i) {
        cash_t val = i->second.get_valueX(token);
        if (val > 0) {
    ...
    if (v.empty()) {
        for (accounts_t::const_iterator i = data->begin(); i != data->end(); ++i) {
            cash_t val = i->second.get_value_oil();
            if (val > 0) {
                v.emplace_back(make_pair(i, val));
            }
        }
...
c::io_accounts_t c::select_source(const hash_t& src, cash_t amount, const hash_t& token) const {
    ...
    if (unlikely(data == nullptr)) {
        ans.warning_code = "KO 93827 data is null.";
        log(ans.warning_code);
        return ans;
    }
    auto i = data->find(src);
    if (i == data->end()) {
        ans.warning_code = "KO 93828 Account not found.";
        log(ans.warning_code, src);
        return ans;
    }
    auto available = i->second.input_eligible3(token);
    if (available.second == 0) {
        ans.warning_code = available.first;
        return ans;
    }
    log("available amount in account", token, available.second);
    if (available.second < amount) {
        if (is_ok(ans.warning_code)) {
            ans.warning_code = "KO 93829 Insufficient balance.";
            log(ans.warning_code);
        }
        return ans;
    }
    ans.warning_code = ok;
    ans.emplace_back(io_account_t(i->first, i->second, amount));
    return ans;
}
...
pair<ko, c::tx_t*> c::tx_make_p2pkh(const tx_make_p2pkh_input& i) {
    ...
    if (input_accounts.empty()) {
        delete ret.second;
        ret.second = nullptr;
        if (is_ko(input_accounts.warning_code)) {
            ret.first = input_accounts.warning_code;
        }
        else {
...
pair<ko, c::tx_t*> c::tx_charge(const hash_t& addr, const cash_t& ramount, const hash_t& rtoken) const {
    return tx_charge_pay(addr, ramount, rtoken, 0, hash_t::zero_);
}
