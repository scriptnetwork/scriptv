...
ko c::verification_completed(pport_t rpport, pin_t pin, request_data_t& request_data) {
    log("verification_completed. endpoint", endpoint(), "fd", get_sock(), "rpport", rpport, "pin", pin);
    {
    ...
    if (is_role_peer() || is_role_sysop()) {
        ...
        if (is_role_peer()) {
            ...
            if (!static_cast<daemon_t&>(daemon).clique.add(*this, false)) {
                ...
                log(r, "disconnect-grid doesn't accept", endpoint(), "sock", get_sock());
                disconnect(0, r);
                return r;
            }
    ...
    if (is_role_device()) {
        ...
        if (!static_cast<daemon_t&>(daemon).grid_dev.add(*this, false)) {
            ...
            log(r, "disconnect-grid_dev doesn't accept", endpoint(), "sock", get_sock());
            disconnect(0, r);
            return r;
        }
        return ok;
    ...
    log(r, "disconnect-unknown role", client::endpoint(), "sock", get_sock());
    disconnect(0, r);
    return r;
}
