...
void c::load_my_protocols() {
    ...
    if (us::gov::io::cfg0::file_exists(file)) {
        auto r = my_protocols_.load(file);
        if (is_ko(r)) {
        }
    }
    //protocols_t my_protocols = w->businesses.published_protocols(false);
    w->businesses.published_protocols(my_protocols, false);
    ...
    switch (pc) {
        ...
        case push_roles: {
            ...
            protocols_t p;
            {
                lock_guard<mutex> lock(mx);
                protocols_t my_protocols = w->businesses.published_protocols(false);
                logdump("push_roles:  my_protocols> ", my_protocols);
                logdump("push_roles:peer_protocols> ", peer_protocols);
...
trader_brief_t c::trader_brief() const {
    trader_brief_t o;
    o.tid = id;
    o.ts_creation = ts_creation;
    o.ts_activity = ts_activity.load();
    assert(w != nullptr);
    o.subhome = w->subhome;
    if (p != nullptr) {
        o.qr = qr_t(remote_endpoint, p->factory_id());
    }
    else {
        o.qr = qr_t(remote_endpoint);
    }
    return move(o);
}
