...
ko c::cert_create(string&& msg, cert_t::options&& options, hash_t& nft) {
    auto r = ca.cert_create(move(msg), move(options), nft);
    return r;
}

ko c::cert_import(cert_t&& cert, hash_t& nft) {
    auto r = ca.cert_import(move(cert), nft);
    return r;
}
...
pair<ko, hash_t> c::initiate(const hash_t parent_tid, const string& datadir, inverted_qr_t&& inverted_qr) { ///<orma flow src="Scan QR Activity/Enter button"/><orma flow src="core0/us/android/app/src/main/java/us/cash/scan.java#submit"/><orma flow src="us::wallet::trader::trades_t::initiate"/>
    ...
    if (unlikely(inverted_qr.endpoint.chan != parent.daemon.channel)) {
        ...
        log(r);
        return make_pair(r, hash_t::zero_);
    }
...
trader_t* c::unlocked_trader_(const hash_t& tid) {
    ...
    if (i == end()) {
        log("new trader", tid);
        auto t = new trader_t(*this, parent.daemon, hash_t::zero_, "");
...
void c::list_trades(ostream& os) const { //deprecated
    lock_guard<mutex> lock(_mx);
    for (auto& i: *this) {
...
ko c::trades_index(trades_index_t& o) const {
    lock_guard<mutex> lock(_mx);
    o.reserve(size());
    for (auto& i: *this) {
        assert(i.second->w != nullptr);
        o.emplace_back(i.second->trader_brief());
    }
    return ok;
}

trades_index_t c::trades_index() const {
    trades_index_t o;
    trades_index(o);
    return move(o);
}
...
void c::help(const string& ind, ostream& os) {
    ...
    fmt::twocol(ind____, "list_protocols/list_businesses", "Lists available trading protocols", os);
    ...
    fmt::twocol(ind____, "load_bookmarks <file> <i>", "Load bookmark from file. Select bookmark #i in in file or -1 for all.", os);
    os << '\n';
}
...
ko c::exec(istream& is) {
    ...
    if (cmd.empty()) {
        ostringstream os;
        exec_help("", os);
        parent.push_OK(hash_t::zero_, os.str());
        return ok;
    }
    ostringstream os;
    if (cmd == "personality") {
        ...
        if (cmd == "set") {
            ...
            if (is_ko(r)) {
                if (r == cert_authority_t::KO_77965) {
                    return parent.push_OK(hash_t::zero_, "Personality didn't change.");
                }
                return r;
            }
            ostringstream os;
            ...
            ca.personality.one_liner(os);
            os << '.';
            return parent.push_OK(hash_t::zero_, os.str());
        }
