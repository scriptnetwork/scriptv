...
void c::save_default_protocolsX() const {
...
pair<ko, hash_t> c::initiate(const hash_t parent_tid, const string& datadir, inverted_qr_t&& inverted_qr, wallet::local_api& w) { //<orma flow dst=""/>
            ...
            }
            {
                ...
                if (i == end()) {
                    ...
                    if (j == i->second.end()) {
                        ...
                        log("found existing entry in the routing table. tid", trade_id, "route", route, "subhome", w->subhome);
...
//                        return r;
        ...
        case trader_t::svc_handshake_c2: { //c == with protocol_selection
            ...
            auto j = i->second.find(route);
            if (j == i->second.end()) {
                log("route not found for tid", trade_id, route);
                ...
                if (j == i->second.end()) {
                    ...
                    log("trade has been initiated here. Updating trades table with the actual route", trade_id, route);
                    auto w = j->second;
                    i->second.erase(0);
                    i->second.emplace(route, w);
                    log("replaced route 0 with", route);
                    log("entry in the routing table:. tid", trade_id, "route", route, "subhome", w->subhome);
                }
            }
            else {
                log("entry in the routing table:. tid", trade_id, "route", route, "subhome", j->second->subhome);
            }
        }
        break;
    }
...
ko c::exec(istream& is, wallet::local_api& w) {
    ...
    if (cmd == "personality") {
        ...
        if (cmd == "set") {
            ...
            if (r == 0) {
                return push_OK(hash_t::zero_, "Personality didn't change.", w);
            }
            ostringstream os;
            os << "New trades will be initiated using personality ";
            personality.one_liner(os);
            os << '.';
            return push_OK(hash_t::zero_, os.str(), w);
        }
...
ko c::from_blob(blob_reader_t& reader) {
    ...
    if (is_ko(r)) return r;
    if (sz > blob_reader_t::max_sizet_containers) [[unlikely]] return blob_reader_t::KO_75643;
    for (int i = 0; i < sz; ++i) {
        log("loading trade #", i);
        uint64_t utid;
        {
