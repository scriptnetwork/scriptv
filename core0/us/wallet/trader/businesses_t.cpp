...
ko c::select(iterator& i) {
    ...
    pair<ko, business_t*> r = parent.daemon.trades.libs.create_business(i->first); //<orma flow dst=""/>
    if (is_ko(r.first)) {
    ...
    #ifdef DEBUG
    {
        ...
        i->second->published_protocols(protocols, false);
        assert(protocols.size() == 1);
        assert(i->first == *protocols.begin());
    }
    #endif
    return ok;
}
...
void c::select_all() {
    ...
    // deleting businesses that didn't load
...
void c::published_protocols(protocols_t& protocols, bool inverse) const { 
    log("published_protocols");
    for (auto& i: *this) {
        i.second->published_protocols(protocols, inverse); /// <orma flow dst="us::wallet::trader::r2r::w2w::business_t"/>
    }
}
...
pair<ko, hash_t> c::initiate(const hash_t parent_tid, const string& datadir, qr_t&& qr) { ///<orma flow src="local_api::handle_trade"/>
    ...
    return parent.daemon.trades.initiate(parent_tid, datadir, move(inverted_qr), parent); //<orma flow dst=""/>
}
