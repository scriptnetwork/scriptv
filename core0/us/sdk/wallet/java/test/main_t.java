...
//    }
    ...
    void check(final blob_t v, final blob_t expected) {
        if (v.size() != expected.size()) {
            System.err.println("Fail. vector sizes differ. Expected " + expected.size() + ", got " + v.size());
            assert false;
        }
        if (v.equals(expected)) return;
        System.err.println("Fail. content of vectors of size " + v.size() + " differ.");
        int i = 0;
        int e = 0;
        while (i < v.size()) {
            System.err.println("" + v.value[i] + ' ' + expected.value[e]);
            ++i; ++e;
        }
        assert false;
    }
    ...
    void test_ev_ser_cash_accounts(accounts_t o) {
        log("test_ev_ser_cash_accounts"); //--strip
        blob_t blob = new blob_t();
        {
            blob_writer_t writer = new blob_writer_t(blob, blob_writer_t.blob_size(o));
            o.to_blob(writer);
...
//        o.write(blob);
        log("o " + o.size()); //--strip
        accounts_t o2 = new accounts_t();
        {
            blob_reader_t reader = new blob_reader_t(blob);
            assert ko.is_ok(reader.read(o2));
        }
//        assert ko.is_ok(o2.read(blob));
        blob_t blob2 = new blob_t();
        {
            blob_writer_t writer = new blob_writer_t(blob2, blob_writer_t.blob_size(o2));
            assert blob2.value != null;
            writer.write(o2);
            assert blob2.value != null;
        }
//        o2.write(blob2);
        log("o2 " + o2.size() + " " + blob2.value); //--strip
        accounts_t o3 = new accounts_t();
        {
            blob_reader_t reader = new blob_reader_t(blob2);
            assert ko.is_ok(reader.read(o3));
        }
//        assert ko.is_ok(o3.read(blob2));
        log("o3 " + o3.size()); //--strip
    ...
    void test_ev_ser_cash_accounts() {
        ...
        log("A2"); //--strip
        {
            accounts_t o = new accounts_t();
            {
                ...
                a.locking_program.value = 4;
                a.box.value.value = 889;
                {
                    a.box.t = new t_t();
                }
                o.put(new hash_t(6059), a);
            }
            test_ev_ser_cash_accounts(o);
        }
        log("A3"); //--strip
        {
            accounts_t o = new accounts_t();
            {
                ...
                {
                    a.box.t = new t_t();
                    a.box.t.put(new hash_t(234756), new cash_t(6489));
                }
                o.put(new hash_t(6059), a);
            }
            {
                account_t a = new account_t();
                a.locking_program.value = 2;
                a.box.value.value = 819;
                {
                    a.box.t = new t_t();
                    a.box.t.put(new hash_t(217766), new cash_t(54489));
                    a.box.f = new f_t();
                    a.box.f.put(new hash_t(3839), new fileattr_t(new string("gdsgfsdg"), new uint32_t(35656)));
                }
                o.put(new hash_t(2059), a);
            }
            test_ev_ser_cash_accounts(o);
        }
    }
