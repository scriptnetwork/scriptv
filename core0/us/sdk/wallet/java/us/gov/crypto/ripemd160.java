...
import static java.lang.Integer.rotateLeft;                                                    // rotateLeft
import java.io.Serializable;                                                                   // Serializable

public class ripemd160 {

    static void log(final String line) {            //--strip
        CFG.log_gov_crypto("ripemd160: " + line);                   //--strip
    }                                               //--strip

    private static final int BLOCK_LEN = 64;
    public static final int output_size = 20;

    public static class hash_t implements Serializable, Comparable<hash_t> {

        public static final hash_t zero_ = new hash_t(0);
        ...
        public hash_t(String b58) {
            ...
            else if (value.length != output_size) {
                value = new byte[output_size];
                zero();
            }
        }
        ...
        @Override public int compareTo(hash_t other) {
        ...
        public boolean is_zero() {
            for (int i = 0; i < output_size; ++i) if (value[i] != 0) return false;
            return true;
        }

        public boolean is_not_zero() {
            for (int i = 0; i < output_size; ++i) if (value[i] != 0) return true;
            return false;
        }
        ...
        public static hash_t decode(String b58) {
            return new hash_t(base58.decode(b58));
        }
        ...
        public int read(final byte[] src, int pos) {
     ...
     */
    public static hash_t hash(byte[] msg) {
        ...
        hash_t result = new hash_t(); //byte[state.length * 4];
        for (int i = 0; i < 20; ++i) {
            result.value[i] = (byte) (state[i / 4] >>> (i % 4 * 8));
        }
        return result;
    }
