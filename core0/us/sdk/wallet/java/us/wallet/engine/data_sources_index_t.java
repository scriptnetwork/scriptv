...
public class data_sources_index_t extends ArrayList<data_sources_index__item_t> implements us.gov.io.seriable {

    private static void log(final String line) {                                 //--strip
        CFG.log_wallet_trader("data_sources_index_t: " + line);                      //--strip
    }                                                                            //--strip

    public void dump(PrintStream os) {
        for (data_sources_index__item_t i: this) {
            os.println(i.name.value + " " + i.address.encode());
        }
    }

    public static serid_t my_serid_id = new serid_t((short)'I');
    ...
    @Override public ko from_blob(blob_reader_t reader) {
        ...
        try {
            for (long i = 0; i < sz.value; ++i) {
                ...
                data_sources_index__item_t item = new data_sources_index__item_t();
                {
                    ko r = item.from_blob(reader);
                    if (is_ko(r)) {
                        log(r.msg); //--strip
                        return r;
                    }
                }
                log("TTE adding entry item=" + item.name.value + " " + item.address.encode()); //--strip
                add(item);
                log("TTE added entry item=" + item.name.value + " " + item.address.encode()); //--strip
            }
        }
        catch (Exception e) {
