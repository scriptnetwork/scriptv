    ...
    public ko from_scanner(Scanner sc) {
        try {
            ...
            if (n > 3) { //ip4+wloc
                log("ip4+wloc"); //--strip
                int pos = 0;
                for (int i = 0; i < n; ++i) pos = epf.indexOf(".", pos) + 1;
                String spkh = epf.substring(0, pos - 1);
                ++pos;
                if (pos < epf.length()) {
                    wloc = epf.substring(pos, epf.length());
                }
                pkh = encode_ip4(spkh);
                ...
                log("spkh=" + spkh); //--strip
                pos++;
                log("pos: " + pos + "; epf.length=" + epf.length() + "; epf=" + epf); //--strip
                if (pos < epf.length()) {
                    log("pos<epf.length(); " + (epf.length() - pos)); //--strip
                    wloc = epf.substring(pos, epf.length());
                    log("wloc=" + wloc); //--strip
                }
                log("spkh=" + spkh + "; wloc=" + wloc); //--strip
            ...
            ko x= new ko("KO 83729 " + e.getMessage());
            log(x.msg); //--strip
            return x;
        }
    }
