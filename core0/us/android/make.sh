...
function check_android_sdk {  #Search K8750R3. SYNC core1/snippet/build_job_make
    echo "checking android_sdk"
    #cmdlinetoolsv=8512546   #https://developer.android.com/studio#command-tools
    cmdlinetoolsv=9477386   #https://developer.android.com/studio#command-tools
    apilevel=32
    ...
    if [[ ! -d ~/android_sdk ]]; then
        echo "recreating android_sdk API ${apilevel}"
        mkdir ~/android_sdk
        ...
        pushd ~/android_sdk >/dev/null
            sdktools=commandlinetools-linux-${cmdlinetoolsv}_latest.zip
            ...
            $sdkmanager "build-tools;${apilevel}.0.0"  #Search R6Tr465. SYNC android/app/build.gradle
