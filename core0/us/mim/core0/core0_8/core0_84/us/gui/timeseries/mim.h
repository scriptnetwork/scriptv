...
#include "menu_app/mim.h"
#include "things/mim.h"

namespace mim::core0::core0_8::core0_85 {
namespace us::gui::timeseries {

    struct vertex_t: template_instance_vertex_t {
        using b = template_instance_vertex_t;

        vertex_t(): b("timeseries", "us::gui::listconf",
            map<string, string>{
                {"MIM file", __FILE__},
                {"datatype", "ArrayList<account_t>"},
                {"itemtype", "account_t"},
                {"title", "Time Series"},
            }) {
            codefiles.add_java();
            add(new menu_app::vertex_t());
            add(new menu_modal::vertex_t());
            add(new things::vertex_t());
