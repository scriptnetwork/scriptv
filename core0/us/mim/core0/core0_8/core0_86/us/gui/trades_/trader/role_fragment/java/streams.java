...
public final class streams extends activity implements datagram_dispatcher_t.handler_t {

    static void log(final String line) {          //--strip
        CFG.log_android("streams: " + line);        //--strip
    }                                             //--strip

    public static class adapter_t extends ArrayAdapter<data_sources_index__item_t> {
        private LayoutInflater inflater;

        public adapter_t(Activity ac, ArrayList<data_sources_index__item_t> data) {
            super(ac, R.layout.streams_list_item, data);
        ...
        @Override public View getView(int position, View convert_view, ViewGroup parent) {
            ...
            data_sources_index__item_t bm = getItem(position);
            if (convert_view == null) {
                convert_view = inflater.inflate(R.layout.streams_list_item, null, true);
            ...
            holder.subject.setText(bm.name.value);
            if (bm.address.is_zero()) {
                holder.tvitem.setText("Disconnected");
                holder.image.setImageResource(R.raw.streams_150);
            }
            else {
                holder.tvitem.setText("Connected to timeseries @ address: " + bm.address.encode());
                holder.image.setImageResource(R.raw.streams_green_150);
            }
    ...
    ArrayList<data_sources_index__item_t> convert(data_sources_index_t o0) {
        return o0;
        /*
        ArrayList<data_sources_index__item_t> o = new ArrayList<data_sources_index__item_t>();
        o.ensureCapacity(o0.size());
        for (data_sources_index__item_t i: this) {
            
            o.add(new pair<hash_t, hash_t>(entry.getKey(), entry.getValue().address));
        }
        return o;
        */
    }
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        set_content_layout(R.layout.streams);
        tabs = findViewById(R.id.tabs);
        lv = (no_scroll_list_view) findViewById(R.id.listviewX);
        assert lv != null;
        adapter = new adapter_t(this, new ArrayList<data_sources_index__item_t>());
...
*/
    void open_conf(final data_sources_index__item_t item) {
    ...
    void launchselectdialogue(String[] sourceshit) {
        Intent intent = new Intent(this, selitems.class);
    ...
    void feed_local_cert(hash_t nft) {
        string result = new string();
        ko r = a.hmi().rpc_peer.call_timeseries_add_local_cert(new us.wallet.cli.rpc_peer_t.timeseries_add_local_cert_in_t(cur.address, nft), result);
        if (is_ko(r)) {
            log(r.msg); //--strip
            toast(r.msg);
            return;
    ...
    void query() {
        if (cur.name.value.equals("human")) {
            query_human();
        }
        else {
            query_stream();
    ...
    void view_timeseries() {
        ...
        toast("TO DO: view timeseries");
    }

    void disconnect_source() {
        string result = new string();
        ko r = a.hmi().rpc_peer.call_data_source_connect(new us.wallet.cli.rpc_peer_t.data_source_connect_in_t(cur.name, new hash_t(0)), result);
        if (is_ko(r)) {
            log(r.msg); //--strip
            toast(r.msg);
            return;
...
//        toast("API called");
    ...
    void connect_source(hash_t addr) {
//        toast("connect " + cur.name.value + " " + addr.encode());
        string result = new string();
        ko r = a.hmi().rpc_peer.call_data_source_connect(new us.wallet.cli.rpc_peer_t.data_source_connect_in_t(cur.name, addr), result);
        if (is_ko(r)) {
            log(r.msg); //--strip
            toast(r.msg);
            return;
...
//        toast("API called");
    ...
    data_sources_index__item_t cur = null;

    void item_click(data_sources_index__item_t item, final int pos) {
        if (mode == 1) {
/*
            String[] options = {"Select this certificate", "View certificate", a.getResources().getString(R.string.cancel)};
            final streams i = streams.this;
...
*/
            ...
            cur = item;
            if (cur.address.is_zero()) {
                String[] options = {"Connect address.", a.getResources().getString(R.string.cancel)};
                new AlertDialog.Builder(streams.this).setTitle(item.name.value + ". Sink port:")
                    .setItems(options, new DialogInterface.OnClickListener() {
                        @Override public void onClick(DialogInterface dialog, int which) {
                            switch(which) {
                                case 0:
                ...
                String[] options = {"Disconnect address.", "Data query + feed", "View time-series", a.getResources().getString(R.string.cancel)};
                new AlertDialog.Builder(streams.this).setTitle(item.name.value + ". Sink port:")
                    .setItems(options, new DialogInterface.OnClickListener() {
                        @Override public void onClick(DialogInterface dialog, int which) {
                            switch(which) {
...
*/
    ...
    //            int mode = args.next<int>(0);
                ...
                data_sources_index_t data_sources_index = new data_sources_index_t();
                //uint8_t mode = new uint8_t(ntab == 0 ? (byte)1 : (byte)0);
                ko r = a.hmi().rpc_peer.call_data_sources(data_sources_index);
                log("endload " + r); //--strip
                if (is_ko(r)) {
                    set_busy__worker(false);
                    log(r.msg); //--strip
                ...
                log("=================================================list " + data_sources_index.size()); //--strip
                list = convert(data_sources_index);
                set_busy__worker(false);
                return;
            }
        });
...
*/
    ...
    void retab() {
        ...
        {
        TabLayout.Tab tab = tabs.newTab().setText("Active");
        tab.setIcon(R.raw.streams);
        //tab.setTag(i);
        tabs.addTab(tab);
        }
        {
...
*/
    ...
    @Override public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CERT_VIEWER_RESULT) {
            if (resultCode != doc_viewer.RESULT_OK) {
...
*/
    ...
    ArrayList<data_sources_index__item_t> list = new ArrayList<data_sources_index__item_t>();
