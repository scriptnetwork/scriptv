...
public class trader extends activity {
...
/*
    @Override public void toolbar_init(toolbar toolbar_) {
        toolbar_.set_stock_button__refresh(new View.OnClickListener() {
            @Override public void onClick(View view) {
                invalidate_data_fetch0();
            }
        });

        toolbar_.set_stock_button__chat(new View.OnClickListener() {
            @Override public void onClick(View view) {
...
*/
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getResources().getString(R.string.trader) + " #" + tid.encode());
...
*/
    ...
    @Override public void on_push(final hash_t target_tid, final uint16_t code, final byte[] payload) {
        log("on_push"); //--strip
//        super.on_push(target_id, code, payload);
    ...
    public void invalidate_data_fetch0() {
        log("invalidate_data_fetch0"); //--strip
        app.assert_ui_thread(); //--strip
        if (!a.has_hmi()) {
    ...
    public void refresh() {
        ...
        if (vf == 1) {
            ...
            if (!create_ui(cur_protocol)) {
                toast("Cannot create UI " + cur_protocol);
            }
            else {
            ...
            try {
                ...
                toast(getResources().getString(R.string.cannotcreatetraderUI));
            }
        }
    ...
    boolean create_ui__katlas(String protocol) {
        ...
        if (protocol.equals("pat2rb pat")) {
            if (specialized_fragment != null) {
                if (specialized_fragment instanceof fragment_pat2rb_pat) {
                    log("right fragment in place"); // --strip
                    return true;
                }
            }
            log("new instance"); // --strip
            specialized_fragment = new fragment_pat2rb_pat();
            return true;
        }
        if (protocol.equals("cdt2hh cdt")) {
            if (specialized_fragment != null) {
                if (specialized_fragment instanceof fragment_cdt2hh_cdt) {
                    log("right fragment in place"); // --strip
                    return true;
                }
            }
            log("new instance"); // --strip
            specialized_fragment = new fragment_cdt2hh_cdt();
            return true;
        }
        if (protocol.equals("cdt2hh hh")) {
            if (specialized_fragment != null) {
                if (specialized_fragment instanceof fragment_cdt2hh_hh) {
                    log("right fragment in place"); // --strip
                    return true;
                }
            }
            log("new instance"); // --strip
            specialized_fragment = new fragment_cdt2hh_hh();
            return true;
        }
        return false;
    }
    ...
    boolean create_ui(String protocol) {
        ...
        protocol_selection_t p = new protocol_selection_t();
        ko r = p.from_string(protocol);
        if (is_ko(r)) {
            return false;
        }
        String clazzname;
        if (p.is_symmetric()) {
            clazzname = "fragment_" + p.first;
        }
        else {
            clazzname = "fragment_" + p.first + "_" + p.second;
        }
        try {
            Class clazz = Class.forName(CFG.namespace + "." + clazzname);
            specialized_fragment = (role_fragment) clazz.newInstance();
        }
        catch (Exception e) {
            log("KO 79856 class not found us.cash." + clazzname); //--strip
            return false;
        }
        return true;
    }
    ...
    public void close_chat__worker() {
        app.assert_worker_thread(); //--strip
        runOnUiThread(new Runnable() {
            @Override public void run() {
                close_chat();
    ...
    void archive() {
        runOnUiThread(new Thread(new Runnable() {
            @Override public void run() {
                toast("This trade has been archived.");
                finish();
            }
        }));
    }
