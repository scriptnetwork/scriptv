...
//MIM  kickoff code hash: 4DJsy3gFbhv2jKAn1jQ4aX4wBcLs (change this hash to force a review)
//MIM  ******************************************************************************
package us.cash;
...
public class trades__conf extends trades__conf__base {

    private static void log(final String line) {           //--strip
        CFG.log_android("trades__conf: " + line);         //--strip
    }                                                      //--strip

    public trades__conf() {
        super();
        log("trades__conf constructor"); //--strip
    }
    ...
    @Override protected void controller__on_create(Bundle saved_state) {
        ...
        log("on_create"); //--strip
        w = (us.cash.scr.trades__conf__widgets) super.w;
        assert w != null; //--strip
    ...
    @Override public us.cash.scr.view__widgets create_widgets() {
        return new us.cash.scr.trades__conf__widgets();
    }
    ...
    void add_tabs() {
        ...
        if (onlytab != -1) {
            switch(onlytab) {
                case 0:
                    add_tab(0, "My QRs", R.raw.qr);
                    break;
                case 1:
                    add_tab(0, "Scan QR", R.raw.qr_scan);
                    break;
                case 2:
                    add_tab(0, "Trades", R.raw.trade);
                    break;
            }
            return;
        }
        add_tab(0, "My QRs", R.raw.qr);
        add_tab(1, "Scan QR", R.raw.qr_scan);
        add_tab(2, "Trades", R.raw.trade);
    }
    ...
    fragment get_fragment(int pos) {
        ...
        if (onlytab != -1) {
            assert pos == 0;
            switch(onlytab) {
                case 0: return new trades__conf__myqr();
                case 1: return new trades__conf__scan();
                case 2: return new trades__conf__active();
                default: return null;
            }
        }
    ...
    us.cash.scr.trades__conf__widgets w = null;

    //MIM begin token 'dyntab_decl'
    public final int num_static_tabs = 3;

    //MIM end token 'dyntab_decl'

    public void select_tab__trades() {
        if (curtab == 2) return;
        curtab = 2;
        tabs_listener_disabled = true;
        w.tabs.getTabAt(curtab).select();
