...
public final class timeseries extends activity implements datagram_dispatcher_t.handler_t {

    static void log(final String line) {          //--strip
        CFG.log_android("timeseries: " + line);        //--strip
    }                                             //--strip

    public static class adapter_t extends ArrayAdapter<hash_t> {
        private LayoutInflater inflater;

        public adapter_t(Activity ac, ArrayList<hash_t> data) {
        ...
        @Override public View getView(int position, View convert_view, ViewGroup parent) {
            ...
            holder.subject.setText(bm.encode());
            holder.tvitem.setText("");
            holder.tvitem2.setText("");
            holder.image.setImageResource(R.raw.timeseries);
            return convert_view;
        }
    }

    ArrayList<hash_t> convert(timeseries_index_t o0) {
        return o0;
    }
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        assert lv != null;
        adapter = new adapter_t(this, new ArrayList<hash_t>());
...
*/
    void open_conf(final hash_t item) {
        
    }

    void item_click(hash_t item, final int pos) {
        if (mode == 1) {
/*
    ...
    void load_list__worker() {
        Thread thread1 = new Thread(new Runnable() {
            @Override public void run() {
                ...
                timeseries_index_t timeseries_index = new timeseries_index_t();
//=====================timeseries_list
...
//======================
                ...
                log("=================================================list " + timeseries_index.size()); //--strip
                list = convert(timeseries_index);
                set_busy__worker(false);
                return;
            }
        });
...
*/
    ...
    @Override public void refresh() {
        ...
        if (mode == 1) {
            setTitle("Select timeseries address");
        }
        else {
            //toolbar.setTitle("Certs");
            setTitle("Time Series. Addresses");
        }
...
*/
    ...
    void new_timeseries() {
        hash_t ans = new hash_t(0);
        uint8_t monotonic = new uint8_t((short)1);
        

        set_busy(true);
        ko r = a.hmi().rpc_peer.call_timeseries_new1(monotonic, ans);
        if (is_ko(r)) {
            set_busy(false);
            log(r.msg); //--strip
            report_ko(r);
        ...
        toast("New timeseries: " + ans.encode());    
        set_busy(false);
...
*/
    ...
    ArrayList<hash_t> list = new ArrayList<hash_t>();
