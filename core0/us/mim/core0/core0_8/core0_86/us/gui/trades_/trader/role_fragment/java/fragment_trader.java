...
public class fragment_trader extends fragment {

    static void log(final String line) {                //--strip
       CFG.log_android("fragment_trader: " + line);     //--strip
    }                                                   //--strip

    public fragment_trader() {
        super(true);
    }
...
//        this.container = container;
        ...
        View view = inflater.inflate(R.layout.fragment_trader, null, false);
        scroll.addView(view);
    ...
    public void select_roles(us.wallet.trader.roles_t roles) {
        ...
        a.new_dlg_builder().setTitle("Select role:")
    ...
    void queryico() {
        ...
        if (tries > 3) {
            ...
            tr.toast(getResources().getString(R.string.givingupretrievingico));
            return;
        }
        ++tries;
    ...
    void on_personality_me(final String cur) {
        ...
        a.new_dlg_builder().setTitle("My Personality: " + cur).setItems(options,
...
log("pasted from main");

        log("remote_bookmarks"); //--strip

        a.main._nodes_mode_custom_tid = tid;
        a.main._nodes_mode_custom = bm;
        a.main._nodes_mode_tab = 1;

        Intent intent = new Intent(a.main, nodes.class);
