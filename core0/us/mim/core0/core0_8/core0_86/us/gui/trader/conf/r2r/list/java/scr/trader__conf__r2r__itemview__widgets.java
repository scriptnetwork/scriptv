...
//MIM  kickoff code hash: 3zr1rLp2yju9GNpSY2Km75gGjA7V (change this hash to force a review)
//MIM  ******************************************************************************
package us.cash.scr;
...
public class trader__conf__r2r__itemview__widgets extends list_view__itemview__widgets {

    private static void log(final String s) {                             //--strip
        CFG.log_scr("trader__conf__r2r__itemview__widgets: " + s);            //--strip
    }                                                                     //--strip
    ...
    public trader__conf__r2r__itemview__widgets(listener_t listener_) {
        listener = listener_;
    }
    ...
    @Override public ViewGroup create_tree(Context ctx) {
        super.create_tree(ctx); {
            canvas_t left = new canvas_t(ctx, 12, 1); {
                ico = new button_t(ctx, R.raw.role, null); {
            ...
            canvas_t right = new canvas_t(ctx, 13, 1); {
                canvas_t rt = new canvas_t(ctx, 13, 0); {
                    ...
                    start_stop_button = new button_t(ctx, R.raw.start, new View.OnClickListener() {
                        @Override public void onClick(View v) {
                            log("on__start_stop__button_click"); //--strip
                            if (listener == null) return;
                            listener.on__start_stop__button_click(list_view__itemview.getBindingAdapterPosition());
                        }
                    }); {  //change icon here
                    }
    ...
    public button_t start_stop_button;
