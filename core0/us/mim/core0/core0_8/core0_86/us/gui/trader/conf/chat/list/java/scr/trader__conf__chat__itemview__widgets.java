...
//MIM  kickoff code hash: 7m9RNQ2iwz1AeG4osXJ8YTRMPYU (change this hash to force a review)
//MIM  ******************************************************************************
package us.cash.scr;
...
public class trader__conf__chat__itemview__widgets extends list_view__itemview__widgets {

    private static void log(final String s) {                             //--strip
        CFG.log_scr("trader__conf__chat__itemview__widgets: " + s);            //--strip
    }                                                                     //--strip
    ...
    public trader__conf__chat__itemview__widgets(listener_t listener_) {
        listener = listener_;
    }
    ...
    @Override public ViewGroup create_tree(Context ctx) {
        super.create_tree(ctx); {
            left = new canvas_t(ctx, 12, 1); {
                ico = new button_t(ctx, R.raw.r2rtab, null); {
            ...
            canvas_t right = new canvas_t(ctx, 13, 1); {
                ...
                tail = new text_view_t(ctx, 1); {
                    tail.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 8);
                }
                right.addView(rt);
                right.addView(tail);
            }
