...
//MIM  kickoff code hash: 2spi3etaJCJZEMhtKGDApp91xrFG (change this hash to force a review)
//MIM  ******************************************************************************
package us.cash;
...
public class accounts__conf extends accounts__conf__base {

    private static void log(final String line) {           //--strip
        CFG.log_android("accounts__conf: " + line);         //--strip
    }                                                      //--strip

    public accounts__conf() {
        super();
        log("accounts__conf constructor"); //--strip
    }
    ...
    @Override protected void controller__on_create(Bundle saved_state) {
        ...
        log("on_create"); //--strip
        w = (us.cash.scr.accounts__conf__widgets) super.w;
        assert w != null; //--strip
    ...
    @Override public us.cash.scr.view__widgets create_widgets() {
        return new us.cash.scr.accounts__conf__widgets();
    }
    ...
    void add_tabs() {
        ...
        if (onlytab != -1) {
            switch(onlytab) {
                case 0:
                    add_tab(0, "Balance", R.raw.wallet);
                    break;
                case 1:
                    add_tab(0, "Addresses", R.raw.ledger);
                    break;
            }
            return;
        }
        add_tab(0, "Balance", R.raw.wallet);
        add_tab(1, "Addresses", R.raw.ledger);
        add_dyn_0_tabs();
    }
    ...
    void add_dyn_0_tabs() {
        ...
        for (String i: dyn_tabs_0) {
            add_tab(pos++, account_t.get_shortened_address(i), R.raw.account);
        }
    }
    ...
    fragment get_fragment(int pos) {
        ...
        if (onlytab != -1) {
            assert pos == 0;
            switch(onlytab) {
                case 0: return new accounts__conf__merged();
                case 1: return new accounts__conf__breakdown();
                default: return null;
            }
        }
        ...
        if (pos < dyn_tabs_0.size()) {
            return new account__conf(dyn_tabs_0.get(pos));
        }
        return null;
    }
    ...
    void from_intent(Intent i) {
        ...
        if (i.hasExtra("onlytab")) {
            ...
            if (i.hasExtra("object_id")) {
                ...
                int dt = i.getExtras().getInt("object_dyntab", 0);
                if (dt == 0) {
                    String o = (String) a.mem_get_object(id);
                    int num_prev = 2;
                    dyn_tabs_0.add(o);
...
*/
    ...
    public void add_dyn_tab_0(String o) {
        log("add_dyn_tab_0 " + o); //--strip
        int p = -1;
        {
            int n = 0;
            for (String i: dyn_tabs_0) {
        ...
        if (p == -1) {
            ...
            o = account_t.get_shortened_address(o);
            add_tab(curtab, o, R.raw.account);
        }
        else {
            curtab = num_prev + p;
        }
    ...
    us.cash.scr.accounts__conf__widgets w = null;
    ...
    public dyntabs_t<String> dyn_tabs_0 = new dyntabs_t<String>();
    //MIM end token 'dyntab_decl'

    void open_addr(final String nft) {
        add_dyn_tab_0(nft);
    }
