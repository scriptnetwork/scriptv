...
bool c::exec_trade_global(const string& command, shell_args& args) {
    ...
    if (command == "list_businesses") {
        using us::wallet::trader::bootstrap::protocols_t;
        protocols_t data;
        rpc_daemon->get_peer().call_list_businesses(data);
        screen::lock_t lock(scr, interactive);
        data.dump(lock.os);;
        return true;
    }
    if (command == "start") {
        ...
        qr.write(blob);
        hash_t tid;
        auto r = rpc_daemon->get_peer().call_trade(rpc_peer_t::trade_in_t(hash_t::zero_, datasubdir, blob), tid);
        if (is_ko(r)) {
    ...
    if (command == "load_bookmarks") {
        ...
        int i = args.next<int>(-2);
        if (i == -1) {
            i = bm2.size();
        }
        if (i < 0) {
            auto r = "KO 92371 Invalid bookmark index.";
            log(r);
            screen::lock_t lock(scr, interactive);
            lock.os << "content of file " << file << '\n'; 
            bm2.dump("    ", lock.os);
            lock.os << r << '\n'; 
            return true;
        }
        ...
        {
            string ans;
            auto r = rpc_daemon->get_peer().call_bookmark_add(rpc_peer_t::bookmark_add_in_t(j->first, j->second), ans);
            if (is_ko(r)) {
                screen::lock_t lock(scr, interactive);
                lock.os << r << '\n'; 
                return true;
...
ko c::tx(shell_args& args) {
    ...
    if (command == "set_supply") {
        ...
        assert(!blob_out.empty());
        scr << us::gov::crypto::b58::encode(blob_out) << '\n';
        return ok;
    }
    if (command == "mint_seal") {
        auto addr = args.next<hash_t>();
        if (addr.is_zero()) {
            auto r = "KO 40302 Invalid address.";
            log(r);
        ...
        blob_t blob_out;
        auto r = rpc_daemon->get_peer().call_mint_seal(rpc_peer_t::mint_seal_in_t(addr, 1), blob_out);
        if (is_ko(r)) {
            return r;
        }
        assert(!blob_out.empty());
    ...
    if (command == "decode") {
        ...
        if (msg == "-f") {
            ...
            blob_in = us::gov::crypto::b58::decode(msg);
        }
        string tx_pretty;
...
void c::help(const params& p, ostream& os) { //moved
    ...
    if (p.advanced) {
        ...
        fmt::twocol(ind____, "mint_seal <address>", "Seal the given mint making a zero-inflation coin.", os);
        os << '\n';
...
ko c::exec_online1(const string& cmd, shell_args& args) {
    ...
    string command = cmd;
    if (command == "transfer" || command == "invoice" || command == "pay" || command == "transfer_from" || command == "set_supply" | command == "mint_seal") {
        command = "tx";
        --args.n;
    }
    if (command == "tx") {
    ...
    if (command == "balance") {
        ...
        if (detailed == 0) {
            ...
            account_t account;
            auto r = rpc_daemon->get_peer().call_merge_accounts(account);
            if (is_ko(r)) {
                return r;
            }
            screen::lock_t lock(scr, interactive);
            account.dump("", detailed, lock.os);
        }
        else {
            string data;
