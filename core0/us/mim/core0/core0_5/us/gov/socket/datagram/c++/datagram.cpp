...
const char* c::KO_9022 = "KO 9022 temporary recv failure.";
...
ko c::dorecv(int sock, size_t len, int& nread, busyled_t& busyled) {
    busyled.reset();
    nread = ::recv(sock, data() + dend, len, 0);
    ...
    busyled.set();
    log("comm", "nread", nread);
    if (nread <= 0) [[unlikely]] {
        if (nread == 0) {
            log("recv 0 bytes.", "dend", dend, "nread", nread, "errno", errno, strerror(errno), "Remote end orderly closed the connection", "fd", sock); //seen here: errno 107 = socket is NOT connected
            return KO_9021;
        }
        if (errno == EAGAIN || errno == EINPROGRESS) {
            log("recv 0 bytes.", "dend", dend, "nread", nread, "errno", errno, strerror(errno), "fd", sock, "EINPROGRESS preserving connection");
            return KO_9022; // ok;
        }
        log("comm", "nread", nread, "unavailable", "KO 4921.1 errno", errno, strerror(errno), "fd", sock);
        return KO_4921;
    }
    assert(nread > 0);
    return ok;
...
ko c::recvfrom(int sock, channel_t channel, busyled_t& busyled) {
    ...
    if (dend < h) {
        if (size() < h) resize(h);
        int nread;
        auto r = dorecv(sock, h - dend, nread, busyled);
        if (is_ko(r)) [[unlikely]] {
            if (r == KO_9022) return ok;
            return r;
        }
        #if CFG_COUNTERS == 1
    ...
    int nread;
    auto r = dorecv(sock, size() - dend, nread, busyled);
    if (is_ko(r)) [[unlikely]] {
        if (r == KO_9022) return ok;
        return r;
    }
    ...
    #endif
    log("comm", "read", dend, size(), completed() ? "OK" : "ok", completed() ? compute_hash() : hash_t::zero_);
    #if CFG_LOGS == 1
        if (completed()) {
