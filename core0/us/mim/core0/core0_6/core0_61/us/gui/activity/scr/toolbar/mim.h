//===-                           S C R I P T  T.V.
//===-                           https://script.tv
//===-
//===-            Copyright (C) 2017-2024 Script Network
//===-            Copyright (C) 2017-2024 manicberet@gmail.com
//===-
//===-                      GNU GENERAL PUBLIC LICENSE
//===-                       Version 3, 29 June 2007
//===-
//===-    This program is free software: you can redistribute it and/or modify
//===-    it under the terms of the GPLv3 License as published by the Free
//===-    Software Foundation.
//===-
//===-    This program is distributed in the hope that it will be useful,
//===-    but WITHOUT ANY WARRANTY; without even the implied warranty of
//===-    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
//===-
//===-    You should have received a copy of the General Public License
//===-    along with this program, see LICENCE file.
//===-    see https://www.gnu.org/licenses
//===-
//===----------------------------------------------------------------------------
//===-
namespace mim::core0::core0_6::core0_61 {
namespace us::gui::activity::scr::toolbar {

    struct vertex_t: layered_class_vertex_t {
        using b = layered_class_vertex_t;

        vertex_t(): b("toolbar", "0", __FILE__) {
            codefiles.add_java();
            codefiles.classname_alg = codefiles_t::vertex_name;
            deepest_baseclass = "LinearLayout";
            subhome = "scr";
        }

        void rewrite_classname(string& classname) const override {
            cout << "rewrite classname " << classname << endl;
            assert(classname == "toolbar");
            classname = "toolbar_t";
        }

    };

}}
