...
#define loglevel "us::trader::r2r::co2vc/vc"
#define logclass "business_t"
#include <us/gov/logs.inc>

using namespace us::trader::r2r::co2vc;
using c = us::trader::r2r::co2vc::vc::business_t;
...
ko c::init(const string& r2rhome, protocol_factories_t& f) {
    ...
    log("co2vc vc init. homebase", home);
    assert(!home.empty());
    {
    ...
    log("co2vc vc initiated @", home);
    return ok;
}

c::protocol_factory_id_t c::protocol_factory_id() const {
    return protocol_factory_id_t(c::protocol::name, "vc");
}
...
void c::list_protocols(ostream& os) const {
    os << c::protocol::name << " vc\n";
}

void c::published_protocols(protocols_t& protocols, bool inverse) const {
    protocols.emplace_back(c::protocol::name, inverse ? "co" : "vc");
}
