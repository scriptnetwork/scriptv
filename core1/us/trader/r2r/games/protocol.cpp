...
using c = us::trader::r2r::games::protocol;

const char* c::rolestr() const { return "player"; }
const char* c::peer_rolestr() const { return "player"; }
const char* c::get_name() const { return name; }

c::protocol(business_t& bz): b(bz) {
    log("created protocol_games_player");
}

c::~protocol() {
    log("destroyed protocol_games_player");
}
